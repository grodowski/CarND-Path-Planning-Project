      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            // Transform ptsx and ptsy to have car coordinates at 0,0
            ...
            // Fit a spline to the 5 waypoints we've just generated
            tk::spline s;
            ...
            // Start with all points from previous path returned by the simulator
            ...
            // Fill up missing waypoints using the spline
            for (int i = 1; i <= 50 - prev_size; i++) {
                double N = target_dist / (.02 * ref_v / 2.24); // mph to m/s
                double x = x_add_on + target_x / N;
                double y = s(x);
                x_add_on = x;
                
                // rotate back to world coords
                double x_world = x * cos(ref_yaw) - y * sin(ref_yaw) + ref_x;
                double y_world = x * sin(ref_yaw) + y * cos(ref_yaw) + ref_y;
